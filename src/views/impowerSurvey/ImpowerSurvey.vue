<template>
  <div style="height: 100%;display: flex;flex-direction: column;">
    <div style="width: 100%;">
      <el-row :gutter="10" style="margin-bottom: 20px">
        <el-col :span="8">
          <div style="display:flex;">
            <el-select v-model="select_term" slot="prepend" placeholder="请选择">
              <el-option label="按设备" value="1"></el-option>
              <el-option label="按时间" value="2"></el-option>
            </el-select>
            <el-input placeholder="请输入内容" clearable v-model="fill_imsi" v-if="select_term==1" class="input-with-select select_item"></el-input>
            <div class="block" v-if="select_term==2">
              <el-date-picker
                v-model="dateArr"
      ...
      <el-row :gutter="20" class="facility_data_style">
        <el-col :span="12" v-for="item in facility_data" :key="item.id">
          <el-col><span></span>{{item.title}}</el-col>
          <el-col :span="6" v-for="items in item.bodyData" :key="items.id">
            <div>{{items.value}}</div>
            <div>{{items.name}}</div>
          </el-col>
        </el-col>
    ...
    <div style="flex: 1;overflow: hidden;padding-bottom: 72px;background: #fff;">
      <el-table :data="tableData" height="100%" border v-loading="loading">
        <el-table-column type="index" width="50" label="序号" v-if="tableData.length!==0"></el-table-column>
